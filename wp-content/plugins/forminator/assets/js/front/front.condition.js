...
(function ($, window, document, undefined) {
	...
	$.extend(ForminatorFrontCondition.prototype, {
		...
		toggle_field: function (element_id, action, type) {
			...
			this.toggle_confirm_email( $element_id );
		},
		...
		// Maybe toggle confirm email field if necessary.
		toggle_confirm_email: function ( $element ) {
			if ( 0 !== $element.length && $element.attr( 'id' ) && -1 !== $element.attr( 'id' ).indexOf( 'email' ) ) {
				var column = $element.closest( '.forminator-col' );
				if ( column.hasClass( 'forminator-hidden' ) ) {
					column.closest( '.forminator-row' ).addClass( 'forminator-hidden' );
				} else {
					column.closest( '.forminator-row' ).removeClass( 'forminator-hidden' );
				}
			}
		},
	});
