...
namespace Smush_Vendor\GuzzleHttp\Handler;

use Smush_Vendor\GuzzleHttp\Exception\ConnectException;
use Smush_Vendor\GuzzleHttp\Exception\RequestException;
use Smush_Vendor\GuzzleHttp\Promise as P;
use Smush_Vendor\GuzzleHttp\Promise\FulfilledPromise;
use Smush_Vendor\GuzzleHttp\Promise\PromiseInterface;
use Smush_Vendor\GuzzleHttp\Psr7\LazyOpenStream;
use Smush_Vendor\GuzzleHttp\TransferStats;
use Smush_Vendor\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx) : PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \Smush_Vendor\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri) : string
    {
        ...
        $redactedUriString = \Smush_Vendor\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \Smush_Vendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \Smush_Vendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
